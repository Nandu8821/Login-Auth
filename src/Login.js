

import {useState,useEffect} from 'react'
import { useNavigate } from 'react-router-dom'

const Home = () => {

     const Navigate=useNavigate()
    const [input, setInput] = useState("")
    const [psw, setPsw] = useState("")
    
  


    const [logindata,setLogindata]=useState(false);


    const CheckData=()=>{ 
     
     const  getuserArr=localStorage.getItem("Myname")

      if(getuserArr && getuserArr.length){
        setLogindata(true);
       
      }

    

}

useEffect(()=>{
CheckData();
},[])


   
  

const handleSubmit =(e)=>{
    e.preventDefault()
      let  url=`http://localhost:4000/student/?input=${input}`;
      fetch(url).then((res)=>{
    return res.json();
   
      }).then((resp)=>{
       

         if(Object.keys(resp).length===0){
            document.getElementById('pwder').innerHTML="Invalid User!";
         }
         else{
            if(resp[0].password===psw)
            {
               window.localStorage.setItem("Myname",resp[0].name);
              window.localStorage.setItem("email",resp[0].email);
                Navigate("/about")
            }
            else{
                document.getElementById("pwder").innerHTML="Invalid password";
                
            }
         }
      })


     
}


   
  return (
    <>

    {
logindata.length === true ? <h1>Error</h1> : <>
<div class="container1">
    <div class="text">
      Login Form !!!!!
    </div>
    <form   >
      <div class="form-row">
        <div class="input-data">
          <input type="text" required onChange={(e)=>setInput(e.target.value)} name='email' value={input} />
          <div class="underline"></div>
          <label for="">Email</label>
        </div>
      
      </div>
 
      <div class="form-row">
 
        <div class="input-data">
          <input type="text" required onChange={(e)=>setPsw(e.target.value)} name='password' value={psw} />
          <div class="underline"></div>
          <label for="">Password</label>
        </div>
      </div>
 
      <div class="form-row">
        <div class="input-data textarea">
 
          <div class="form-row submit-btn">
            <div class="input-data">
              <div class="inner"></div>
              <input type="submit" value="Login" onClick={handleSubmit} />
            </div>
          </div>
        </div>
      </div>
    </form>

    <p id='pwder'></p>
   
  </div>

 
 

</>
    }
 
    </>
  )
}

export default Home
